...
def _qualifies_for_offer(record: OfferRecord, amount_source: OfferRecord) -> bool:
    if not _flag(record, "active"):
        return False
    if _flag(record, "suspended"):
        return False
    if _amount(amount_source, "subtotal") < 100:
        return False
...
def qualifies_for_trial_discount(customer: OfferRecord, invoice: OfferRecord) -> bool:
    return _qualifies_for_offer(customer, invoice)


def can_receive_intro_offer(account: OfferRecord, cart: OfferRecord) -> bool:
    return _qualifies_for_offer(account, cart)
